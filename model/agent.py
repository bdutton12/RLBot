  ...
  """
    ...
    def choose_action(self, observation):
        state = torch.tensor([observation], dtype=torch.float).to(
            self.actor.device
        )  # The game state is converted to a PyTorch tensor

        distribution = self.actor(
            state
        )  # Calls the 'forward' method of the actor class, getting a distribution of classes and their likelihood
        value = self.critic(
            state
        )  # Calls the 'forward' method of the critic class, getting a score of how 'good' the action is
        action = (
            distribution.sample()
        )  # Retrieve a sample from the distribution of categories

        probs = torch.squeeze(
            distribution.log_prob(action)
        ).item()  # Get the associated probability of success for the action
